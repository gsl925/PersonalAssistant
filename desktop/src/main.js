...
const { ingestText, ingestUrl, ingestFilePath } = require("./api");
...
if (!gotLock) {
  ...
  app.whenReady().then(() => {
    ...
    console.log(`[main] hotkey registration — screenshot(Ctrl+Shift+S): ${screenshotOk}, note(Ctrl+Shift+N): ${noteOk}`);
    if (!screenshotOk || !noteOk) {
